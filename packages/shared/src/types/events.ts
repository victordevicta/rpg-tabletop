import type { BaseDocument, TokenData } from './document';
...
export interface CombatStartPayload {
  worldId: string;
  sceneId: string;
  combatants: Array<{ tokenId: string; name: string; initiative: number }>;
}
...
export interface ServerToClientEvents {
  ...
  'document:delete': (payload: { worldId: string; documentId: string; type: string }) => void;
  'token:move': (payload: TokenMovePayload) => void;
  'scene:activate': (payload: SceneActivatePayload) => void;
  'chat:message': (payload: ChatSendPayload & { id: string; createdAt: string }) => void;
  'roll:create': (payload: RollCreatePayload & { id: string; createdAt: string }) => void;
  ...
  'world:userJoined': (payload: { userId: string; name: string }) => void;
  'world:userLeft': (payload: { userId: string }) => void;
  error: (payload: { message: string }) => void;
}

export interface ClientToServerEvents {
  'world:join': (payload: WorldJoinPayload) => void;
  'world:leave': (payload: { worldId: string }) => void;
  'token:move': (payload: TokenMovePayload) => void;
